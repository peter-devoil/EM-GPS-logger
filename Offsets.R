...
# Apply an offset (from the EM boom) to the position of each observation,
# so that the points are in the plot boundaries, not the buggy cabin.

dataPath <- "Dualem1HS_26062024_Wheat.All.csv"

# (right) distance of boom to GPS in m
boomRight <-3.5
boomTrailing <- 1

# whether to remove obsns when movement has stopped
removeDuplicates <- T
# distance within X to consider as stopped (m)
stoppingDistance <- 0.33
...
# Read data and massage columns
df <- read.csv(dataPath, sep = ",", header=T) %>%
...
#ggplotly(ggplot(df) + geom_point(aes(x=Track, y=Track2, colour=nNear)))
#ggplotly(ggplot(df) + 
#           geom_point(aes(x=Longitude, y=Latitude, colour=nNear))+
#           geom_point(aes(x=Longitude, y=Latitude, colour=nNear), data = df %>% filter(nNear > 10)))
...
if (removeDuplicates) {
  ...
  for (i in 1:nrow(df)) {
    for (j1 in i:1) {
      d <- distGeo(c(df$Longitude[i], df$Latitude[i]), 
                   c(df$Longitude[j1], df$Latitude[j1]))
      if (d > stoppingDistance / 2) { # all obsns within 0.5m prior
        break
      }
    }
    for (j2 in i:nrow(df)) {
      d <- distGeo(c(df$Longitude[i], df$Latitude[i]), 
                   c(df$Longitude[j2], df$Latitude[j2]))
      if (d > stoppingDistance / 2) { # all obsns within 0.5m after
        break
      }
    }
    if (j2 > j1) {
...
#ggplotly(ggplot(df) + geom_point(aes(x=Longitude, y=Latitude, colour=nNear)))
  
# write a .shp in the same place
write_sf(st_as_sf(df, coords = c("Longitude", "Latitude"), crs = "WGS84"),
         dsn=file.path(dirname(dataPath),gsub(".csv", ".shp", basename(dataPath))))
